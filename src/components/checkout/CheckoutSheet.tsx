    ...
    const handleSignUp = async (values: SignUpFormValues) => {
        startTransition(async () => {
            ...
            if (error) {
                ...
                onOpenChange(false); // Close the sheet on successful signup
                // The user is now logged in, the main page will handle the next click
            }
        });
    };
        ...
        <Sheet open={isOpen} onOpenChange={onOpenChange}>
            <SheetContent className="sm:max-w-xl w-full p-0 flex flex-col">
                <ScrollArea className="flex-1">
                    <div className="p-6">
                        <SheetHeader>
                            <SheetTitle className="text-2xl flex items-center gap-2"><UserPlus/>Crea tu cuenta para continuar</SheetTitle>
