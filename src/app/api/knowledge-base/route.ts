...
export const GET = apiHandler(async (req: NextRequest) => {
    ...
    let query = adminDb.collection(KNOWLEDGE_BASE_COLLECTION);

    if (sessionId) {
        // If a session ID is provided, fetch documents matching that session ID.
        // We no longer order by chunk_number here to avoid needing a composite index.
        const snapshot = await query.where('metadata.sessionId', '==', sessionId).get();
        if (snapshot.empty) {
        ...
        // Default to global admin knowledge base if no session ID, ordered by creation.
        // This query works without a special index.
        const snapshot = await query.where('metadata.source', '==', 'admin_kb').get();
        if (snapshot.empty) {
            return ApiResponse.success([]);
        }
