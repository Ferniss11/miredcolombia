...
import { Loader2, Bot, TestTube, Power, PowerOff, Sparkles, Building2, BarChart, BadgeDollarSign, MessageCircle, Star, ShoppingCart, Camera } from "lucide-react";
...
const ToolCard = ({ icon: Icon, title, description, isConnected, onConnect, onDisconnect, isConnectPending, disabled, iconColor }: ToolCardProps) => (
    <div className={cn(
        ...
        <div className={cn("p-3 rounded-full bg-muted/80 mb-2", isConnected && "bg-primary/10")}>
            <Icon className={cn("h-7 w-7", isConnected ? "text-primary" : "text-muted-foreground", iconColor)} />
        </div>
